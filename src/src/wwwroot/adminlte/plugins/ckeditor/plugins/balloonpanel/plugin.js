...
*/
(function () {
    var t = !1; CKEDITOR.plugins.add("balloonpanel", { init: function () { t || (CKEDITOR.document.appendStyleSheet(this.path + "skins/" + CKEDITOR.skin.name + "/balloonpanel.css"), t = !0) } }); CKEDITOR.ui.balloonPanel = function (a, b) {
    this.editor = a; CKEDITOR.tools.extend(this, { width: 360, height: "auto", triangleWidth: 20, triangleHeight: 20, triangleMinDistance: 40 }, b, !0); this.templates = {}; for (var d in this.templateDefinitions) this.templates[d] = new CKEDITOR.template(this.templateDefinitions[d]); this.parts = {}; this.focusables =
        ...
        build: function () {
            ...
            function a(a) { var c = a.getClientRect(), k = b.getScrollPosition(); r || a.equals(d) ? (c.top += k.y, c.left += k.x) : (a = d.getClientRect(), c.top = a.top + c.top + k.y, c.left = a.left + c.left + k.x); c.right = c.left + c.width; c.bottom = c.top + c.height; return c } var b, d, p, r, t = {
                right: "left", top: "bottom",
                topLeft: "bottomLeft", topRight: "bottomRight", bottom: "top", bottomLeft: "topLeft", bottomRight: "topRight", left: "right"
            }; return function (w, c) {
                if (c instanceof CKEDITOR.dom.element || !c) c = { focusElement: c }; c = CKEDITOR.tools.extend(c, { show: !0 }); !0 === c.show && this.show(); this.fire("attach"); b = CKEDITOR.document.getWindow(); d = this.editor.window.getFrame(); p = this.editor.editable(); r = p.isInline(); var k = this.getWidth(), v = this.getHeight(), g = a(w), m = a(r ? p : d), h = b.getViewPaneSize(), e = b.getScrollPosition(), h = {
                    top: Math.max(m.top,
                        e.y), left: Math.max(m.left, e.x), right: Math.min(m.right, h.width + e.x), bottom: Math.min(m.bottom, h.height + e.y)
                }; r && (h = this._getViewPaneRect(b), h.right += this.triangleWidth, h.bottom += this.triangleHeight); this._adjustElementRect(g, r ? h : m); var m = k * v, g = this._getAlignments(g, k, v), f, l, n; for (n in g) {
                    e = v; l = { top: g[n].top, left: g[n].left }; l.right = l.left + k; l.bottom = l.top + e; var e = g[n], q = l, u = h; l = Math.max(0, Math.min(q.right, u.right) - Math.max(q.left, u.left)); q = Math.max(0, Math.min(q.bottom, u.bottom) - Math.max(q.top, u.top));
                    e = e.areaDifference = m - l * q; if (0 === e) { f = n; break } f || (f = n); e < g[f].areaDifference && (f = n)
                } this.move(g[f].top, g[f].left); f = f.split(" "); this.setTriangle(t[f[0]], f[1]); !1 !== c.focusElement && (c.focusElement || this.parts.panel).focus()
            }
        }(), resize: function (a, b) { this.rect.width = a; this.rect.height = b; this.parts.panel.setStyles({ width: CKEDITOR.tools.cssLength(a), height: CKEDITOR.tools.cssLength(b) }) }, getWidth: function () { return "auto" === this.rect.width ? this.parts.panel.getClientRect().width : this.rect.width }, getHeight: function () {
            return "auto" ===
                this.rect.height ? this.parts.panel.getClientRect().height : this.rect.height
        }, setTriangle: function (a, b) {
            var d = this.parts.triangleOuter, p = this.parts.triangleInner; this.triangleSide && (d.removeClass("cke_balloon_triangle_" + this.triangleSide), d.removeClass("cke_balloon_triangle_align_" + this.triangleAlign), p.removeClass("cke_balloon_triangle_" + this.triangleSide)); this.triangleSide = a; this.triangleAlign = b; d.addClass("cke_balloon_triangle_" + a); d.addClass("cke_balloon_triangle_align_" + b); p.addClass("cke_balloon_triangle_" +
                a)
        ...
        setTitle: function (a) { this.parts.title.setHtml(a) }, _getAlignments: function (a, b, d) {
            return {
                "right vcenter": { top: a.top + a.height / 2 - d / 2, left: a.right + this.triangleWidth }, "left vcenter": { top: a.top + a.height / 2 - d / 2, left: a.left - b - this.triangleWidth }, "top hcenter": { top: a.top - d - this.triangleHeight, left: a.left + a.width / 2 - b / 2 }, "top left": { top: a.top - d - this.triangleHeight, left: a.left + a.width / 2 - this.triangleMinDistance }, "top right": { top: a.top - d - this.triangleHeight, left: a.right - a.width / 2 - b + this.triangleMinDistance }, "bottom hcenter": {
                    top: a.bottom +
                        this.triangleHeight, left: a.left + a.width / 2 - b / 2
                }, "bottom left": { top: a.bottom + this.triangleHeight, left: a.left + a.width / 2 - this.triangleMinDistance }, "bottom right": { top: a.bottom + this.triangleHeight, left: a.right - a.width / 2 - b + this.triangleMinDistance }
            }
        }, _adjustElementRect: function (a, b) { a.left = Math.max(b.left, Math.min(b.right - 1, a.left)); a.right = Math.max(b.left, Math.min(b.right, a.right)); a.top = Math.max(b.top, Math.min(b.bottom - 1, a.top)); a.bottom = Math.max(b.top, Math.min(b.bottom, a.bottom)) }, _getViewPaneRect: function (a) {
            var b =
                a.getScrollPosition(); a = a.getViewPaneSize(); return { top: b.y, bottom: b.y + a.height, left: b.x, right: b.x + a.width }
        }
